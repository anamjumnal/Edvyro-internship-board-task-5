...
- **App (frontend + API, same origin):** https://Edvyro-internship-board-task-4-production.up.railway.app
- **Repository:** _(add your GitHub repo link here)_
- **Two-minute walkthrough:** _(add video link here)_
...
- **Hosting:** Railway (backend + static files together)
...
5. Unsafe portfolio URL scheme (javascript:alert(1))
   POST /api/applications  →  HTTP 400
   {"status":"error","message":"Validation failed","errors":["Invalid portfolio URL format"]}

6. Sixth application within 15 minutes from the same source
...
- `index.html`, `script.js` — frontend (fetches from the live Railway API)
